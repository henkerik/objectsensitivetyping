...
class LowerTarget : public virtual AST::Parent, virtual public GC_obj
{	
protected:
    bool enableWrite;
    bool enableRead;
    bool isNestedVariable;
    int depth;
...
//        cout << "0..." << endl;
    ...
    void children_variable(Variable* in)
    {
        if (isNestedVariable) {
            in->attrs->set_true ("uu.is_nested");
        } else {
            if (in->array_indices->size() > 0 && in->target != NULL) {
                in->attrs->set_true ("uu.is_nested");
            } else {
                in->attrs->set_false ("uu.is_nested");
            }
        }

        bool backup = isNestedVariable;
        ...
        isNestedVariable = backup;
    }
        
    void children_return(Return* in)
    {
        bool backupRead = enableRead;
        enableRead = true;
        in->expr = transform_expr(in->expr);
        enableRead = backupRead;
    }
    
    void children_throw(Return* in)
    {
        bool backupRead = enableRead;
        enableRead = true;
        in->expr = transform_expr(in->expr);
    ...
    void children_continue(Return* in)
    {
        bool backupRead = enableRead;
        enableRead = true;
        in->expr = transform_expr(in->expr);
    ...
    void children_pre_op(Pre_op* in)
    {
        in->op = transform_op(in->op);
        
        bool backupWrite = enableWrite;
        bool backupRead  = enableRead;
        
        enableWrite = true;
        enableRead  = true;
        in->variable = transform_variable(in->variable);
        enableWrite = backupWrite;
        enableRead  = backupRead;
    }
    
    void children_post_op(Post_op* in)
    {
        bool backupWrite = enableWrite;
        bool backupRead  = enableRead;
...
//        cout << "3.." << in << endl;
        ...
        if (in->attrs->is_true ("uu.ast_shredder.dont_shred"))
    		return in;

        
        Variable* prev = in;
        
        int num_pieces = (in->target != NULL ? 1 : 0) 
                       + in->array_indices->size()
                       - (in->attrs->is_true("phc.ast_shredder.need_addr") ? 1 : 0);
        
//        cout << "num pieces: " << num_pieces << endl;
        
        bool isNested = in->attrs->is_true ("uu.is_nested");
        
        if (in->target != NULL) {
//            cout << "Target not null: enableRead: " << enableRead << ", enableWrite: " << enableWrite << endl;
            
            Variable* temp = fresh_var("TSNNt");
            
            // Reading
            if (enableRead || isNested) {
            ...
            if (post_pieces != NULL) {
                // Writing
                if (enableWrite || isNested) { 
//                cout << "size of post pieces: " << post_pieces->size() << endl;
                    
  //                  cout << "pushing field write stmts: target: " << in->target << ", variable name: " << in->variable_name << endl;

                    post_pieces->push_front(new Eval_expr(new Assignment(
            		    new Variable (
            			    in->target->clone(),
            			    in->variable_name->clone(),
            			    new Expr_list
 ...
 //               cout << "post pieces still null" << endl;
            }
...
//        cout << "before loop, num_pieces: " << num_pieces << endl;
...
//            cout << "array_index" << array_index << endl;	
    		...
    		Variable* temp = fresh_var("TSNNi");
    		
    		if (enableRead || isNested) {
//                cout << "pushing array read stmts" << endl;
    		...
    		if (enableWrite || isNested) {
//                cout << "pushing array write stmts" << endl;
...
namespace AST 
{
    ...
    class LowerTarget : public virtual AST::Lower_expr {
        public:
            void children_php_script(PHP_script* in)
            {
            	//Annotate ann;
