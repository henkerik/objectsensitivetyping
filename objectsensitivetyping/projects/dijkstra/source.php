 ...
 * @package Opus11
 */
	class Association extends AbstractComparable
	{
		/**
     ...
     */
		public function __toString()
		{
			$result = 'Association{' . $this->key->__toString();
			if ($this->value !== NULL) 
 ...
 */
     ...
     */
		function enqueue(IComparable $obj);
		/**
 ...
 * @package Opus11
 */
	abstract class AbstractPriorityQueue extends AbstractContainer implements IPriorityQueue
	{
 ...
 * @package Opus11
 */
	class BinaryHeap_Iterator extends AbstractIterator
	{
		/**
     ...
     */
		public function current()
		{
			...
			return $array->offsetGet($this->position);
		}
 ...
 * @package Opus11
 */
	class BinaryHeap extends AbstractPriorityQueue
	{
		/**
     * The array.
     */
    ...
    **/
		...
		{
			while ($this->count > 0) 
				$this->array->offsetSet($this->count--, NULL);
		}
		//}>a
     ...
     */
		public function enqueue(IComparable $obj)
		{
			...
			++$this->count;
			$i = $this->count;
			while ($i > 1 && gt($this->array->offsetGet(intval($i / 2)), $obj)) {
				$this->array->offsetSet($i, $this->array->offsetGet(intval($i / 2)));
				$i = intval($i / 2);
			}
			$this->array->offsetSet($i, $obj);
		}
		//}>c
     ...
     */
		public function dequeueMin()
		{
			if ($this->count == 0) 
				throw new ContainerEmptyException();
			$result = $this->array->offsetGet(1);
			$last = $this->array->offsetGet($this->count);
			--$this->count;
			$i = 1;
			while (2 * $i < $this->count + 1) {
				$child = 2 * $i;
				if ($child + 1 < $this->count + 1 && lt($this->array->offsetGet($child + 1), $this->array->offsetGet($child))) 
					$child += 1;
				if (le($last, $this->array->offsetGet($child))) 
					break;
				$this->array->offsetSet($i, $this->array->offsetGet($child));
				$i = $child;
			}
			$this->array->offsetSet($i, $last);
			return $result;
		}
		//}>e
     ...
     */
		public function accept(IVisitor $visitor)
		{
			for($i = 1; $i < $this->count + 1; ++$i) {
				if ($visitor->isDone()) 
					return;
				$visitor->visit($this->array->offsetGet($i));
			}
		}
 ...
 */
	...
	function dijkstrasAlgorithm(IDigraph $g, $s)
	{
		$n = $g->getNumberOfVertices();
		$table = new BasicArray($n);
		for($v = 0; $v < $n; ++$v) 
			$table->offsetSet($v, new Entry());
		$table->offsetGet($s)->distance = 0;
		$queue = new BinaryHeap($g->getNumberOfEdges());
		$queue->enqueue(new Association(new BoxedInteger(0), $g->getVertex($s)));
		while (!$queue->isEmpty()) {
			$assoc = $queue->dequeueMin();
			$v0 = $assoc->getValue();
			$n0 = $v0->getNumber();
			if (!$table->offsetGet($n0)->known) {
				$table->offsetGet($n0)->known = true;
				$it = $v0->getEmanatingEdges()->getIterator();
				$it->rewind();
				while ($it->valid()) {
					$edge = $it->current();
					$v1 = $edge->getMate($v0);
					$n1 = $v1->getNumber();
					$wt = $edge->getWeight();
					$d = $table->offsetGet($n0)->distance + unbox($wt);
					if ($table->offsetGet($n1)->distance > $d) {
						$table->offsetGet($n1)->distance = $d;
						$table->offsetGet($n1)->predecessor = $n0;
						$queue->enqueue(new Association(new BoxedInteger($d), $v1));
					}
					$it->next();
				}
			}
		...
		for($v = 0; $v < $n; ++$v) 
			$result->addVertex($v, new BoxedInteger($table->offsetGet($v)->distance));
		for($v = 0; $v < $n; ++$v) 
			if ($v != $s) 
				$result->addEdge($v, $table->offsetGet($v)->predecessor);
		return $result;
	}
	
	$g = new DigraphAsLists(32);
	$g->addVertex(0, new BoxedInteger(123));
	$g->addVertex(1, new BoxedInteger(234));
	$g->addVertex(2, new BoxedInteger(345));
	$g->addEdge(0, 1, new BoxedInteger(3));
	$g->addEdge(0, 2, new BoxedInteger(1));
	$g->addEdge(1, 2, new BoxedInteger(4));
	
	$g2 = dijkstrasAlgorithm($g, 0);
	printf("%s\n", $g2->__toString());
